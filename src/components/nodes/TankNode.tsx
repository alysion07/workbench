/**
 * TANK Node Component
 * Based on BranchNode — same dynamic junction handles, different visual identity
 */

import { memo, useMemo, useCallback, useEffect } from 'react';
import { Handle, Position, NodeProps, NodeResizer, useUpdateNodeInternals } from 'reactflow';
import { Box, Typography } from '@mui/material';
import { MARSNodeData, TankParameters } from '@/types/mars';
...
const TankNode: React.FC<NodeProps<MARSNodeData>> = ({ data, selected, id }) => {
  ...
  const params = parameters as Partial<TankParameters>;
  ...
  const handles = useMemo(() => {
    ...
    // Auto-connect (Face 0): hidden center handles
    result.push(
      <Handle
  ...
  return (
    <>
      <NodeResizer
        ...
        handleStyle={{ width: 8, height: 8, borderRadius: 2, backgroundColor: '#0097A7', zIndex: 10 }}
      ...
      {isSvgShape(appearance.shape) && svgItem ? (
        ...
        <Box
          sx={{
            width: displayWidth,
            height: displayHeight,
            backgroundColor: appearance.backgroundColor || '#E0F7FA',
            clipPath,
            borderRadius,
            border: clipPath ? 'none' : (selected ? '2px solid #0097A7' : '1px solid #80DEEA'),
            display: 'flex',
            ...
            boxShadow: selected ? '0 0 0 2px rgba(0,151,167,0.3)' : 'none',
            overflow: 'hidden',
            position: 'relative',
            '&:hover': {
