/**
 * TURBINE Node Component
 * Specialized Branch — dynamic junction handles (same pattern as BranchNode)
 */

import { memo, useMemo, useCallback, useEffect } from 'react';
import { Handle, Position, NodeProps, NodeResizer, useUpdateNodeInternals } from 'reactflow';
import { Box, Typography } from '@mui/material';
import { MARSNodeData, TurbineParameters } from '@/types/mars';
...
const TurbineNode: React.FC<NodeProps<MARSNodeData>> = ({ data, selected, id }) => {
  ...
  const params = parameters as Partial<TurbineParameters>;
  ...
  // Inlet/Outlet 분리
  const inletJunctions = useMemo(() => junctions.filter(j => j.direction === 'inlet'), [junctions]);
  const outletJunctions = useMemo(() => junctions.filter(j => j.direction === 'outlet'), [junctions]);

  // 핸들 생성 — Branch와 동일한 target-jN / source-jN 패턴
  ...
  return (
    <>
      <NodeResizer
        ...
        minWidth={30}
        minHeight={30}
        lineStyle={{ borderColor: '#607D8B', zIndex: 10 }}
        handleStyle={{ width: 8, height: 8, borderRadius: 2, backgroundColor: '#607D8B', zIndex: 10 }}
      ...
      {isSvgShape(appearance.shape) && svgItem ? (
        ...
        <Box
          sx={{
            ...
            clipPath,
            borderRadius,
            border: clipPath ? 'none' : (selected ? '2px solid #607D8B' : '1px solid #bdbdbd'),
            display: 'flex',
            ...
            boxShadow: selected ? '0 0 0 2px rgba(96,125,139,0.3)' : 'none',
            overflow: 'hidden',
            position: 'relative',
            '&:hover': {
